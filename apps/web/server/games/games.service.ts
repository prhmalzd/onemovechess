...
export const gamesService = {
  ...
  async getAvailableBoards(playerId: string, offset: number) {
    ...
    return prisma.$transaction(async (database) => {
      await requirePlayerProfile(database, playerId);
      await clearExpiredReservations(database);
      const rows = await database.$queryRaw<Array<{ id: string; current_fen: string; current_ply: bigint | number; player_count: bigint | number; updated_at: Date }>>`
        select g.id, g.current_fen, g.current_ply::int as current_ply, g.updated_at,
      ...
      const boards = rows.slice(0, pageSize).map((row) => ({
        id: row.id, currentFen: row.current_fen, currentPly: Number(row.current_ply), playerCount: Number(row.player_count), updatedAt: row.updated_at.toISOString(),
      }));
