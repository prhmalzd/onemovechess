...
export function AccountModal({ allowSignIn, onClose }: { allowSignIn: boolean; onClose: () => void }) {
  ...
  return <div aria-labelledby="account-title" aria-modal="true" className="modal-backdrop" role="dialog">
    <section className="save-progress-modal account-modal">
      ...
      <h2 id="account-title">{isCreating ? 'Save your progress' : 'Sign in'}</h2>
      <p>{isCreating ? 'Create a username and password to keep this player, its moves, and its boards when you switch devices.' : 'Sign in to continue with your saved player.'}</p>
      ...
      <label className="account-field" htmlFor="account-username">Username<input autoCapitalize="none" autoComplete="username" id="account-username" maxLength={20} onChange={(event) => setUsername(event.target.value)} placeholder="chess_player" value={username} /></label>
      <label className="account-field" htmlFor="account-password">Password<input autoComplete={isCreating ? 'new-password' : 'current-password'} id="account-password" onChange={(event) => setPassword(event.target.value)} type="password" value={password} /></label>
      {isCreating && <><label className="account-field" htmlFor="account-password-confirm">Confirm password<input autoComplete="new-password" id="account-password-confirm" onChange={(event) => setConfirmPassword(event.target.value)} type="password" value={confirmPassword} /></label><ul className="password-rules">{passwordRequirementLabels.map((rule) => <li className={isStrongPassword(password) ? 'password-rule--met' : ''} key={rule}>{rule}</li>)}</ul><div className="chess-captcha"><strong>Quick chess check</strong><p>Move the knight from d3 to b4.</p><div aria-label="Move the knight to b4" className="captcha-board" role="grid">{captchaSquares.map((square, index) => { const hasKnight = square === 'd3' && captchaStage !== 'complete'; const isTarget = square === 'b4'; const isLight = (index % 5 + Math.floor(index / 5)) % 2 === 0; return <button aria-label={hasKnight ? 'Knight on d3' : square} className={`${isLight ? 'captcha-square--light' : 'captcha-square--dark'} ${captchaStage === 'knight-selected' && square === 'd3' ? 'captcha-square--selected' : ''} ${isTarget ? 'captcha-square--target' : ''}`} key={square} onClick={() => moveCaptcha(square)} role="gridcell" type="button">{hasKnight ? '♞' : ''}</button>; })}</div>{captchaStage === 'complete' && <p className="captcha-success">Knight moved — nice.</p>}</div></>}
      {error && <p className="auth-error" role="alert">{error}</p>}
      <button className="account-submit" disabled={isSubmitting} onClick={() => { void submit(); }} type="button">{isSubmitting ? 'Saving…' : isCreating ? 'Create account' : 'Sign in'}</button>
      <button className="dismiss-auth" disabled={isSubmitting} onClick={onClose} type="button">Not now</button>
    </section>
  </div>;
}
