...
function AvailableBoardsSidebar({ boards, offset, nextOffset, isClaimingBoardId, isCreatingBoard, isLoading, isOpen, onClaim, onCreate, onRandom, onOffsetChange, onToggle, variant = 'sidebar' }: { boards: AvailableBoard[]; offset: number; nextOffset: number | null; isClaimingBoardId: string | null; isCreatingBoard: boolean; isLoading: boolean; isOpen: boolean; onClaim: (gameId: string) => void; onCreate: () => void; onRandom: () => void; onOffsetChange: (offset: number) => void; onToggle: () => void; variant?: 'sidebar' | 'chooser' }) {
  ...
  return <aside aria-label="Available boards" className={className}>
    ...
    {isOpen && <div className="available-boards-sidebar__content">{isLoading ? <p className="muted">Looking for boards…</p> : boards.length ? <><p className="available-boards-sidebar__intro">Choose a board, or let us pick one for you.</p><button className="random-board-button" disabled={isClaimingBoardId !== null || isCreatingBoard} onClick={onRandom} type="button">♞ Choose a random board</button><div className="available-boards__list">{boards.map((board, index) => <button className="available-board" disabled={isClaimingBoardId !== null || isCreatingBoard} key={board.id} onClick={() => onClaim(board.id)} type="button"><div className={`available-board__preview chessboard--locked ${pieceStyle === 'monochrome' ? 'piece-style--monochrome' : ''}`}><Chessboard options={{ id: `available-${board.id}`, position: board.currentFen, allowDragging: false, showNotation: false, darkSquareStyle: { backgroundColor: theme.dark }, lightSquareStyle: { backgroundColor: theme.light } }} /></div><span className="available-board__details"><span className="available-board__number">Board {String(offset + index + 1).padStart(2, '0')}</span><strong>{new Chess(board.currentFen).turn() === 'w' ? 'White to move' : 'Black to move'}</strong><small>Move {board.currentPly}</small></span>{isClaimingBoardId === board.id && <i>Joining…</i>}</button>)}</div><div className="available-boards__controls"><button disabled={offset === 0} onClick={() => onOffsetChange(Math.max(0, offset - 4))} type="button">← Earlier</button><button disabled={nextOffset === null} onClick={() => { if (nextOffset !== null) onOffsetChange(nextOffset); }} type="button">More boards →</button></div></> : <div className="available-boards-sidebar__empty"><p>No playable boards are available right now.</p><button className="create-board-button" disabled={isCreatingBoard} onClick={onCreate} type="button">{isCreatingBoard ? 'Creating board…' : 'Create new board'}</button></div>}</div>}
  </aside>;
}
