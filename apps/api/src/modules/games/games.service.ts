...
function serializeGame(game: GameDetails) {
  return {
    ...
    participants: game.participants.map((participant) => ({
      ...
      lastMovePly: participant.lastMovePly,
      joinedAt: participant.joinedAt.toISOString(),
      timedOutAt: participant.timedOutAt?.toISOString() ?? null,
    })),
    reservation: game.reservation ? {
      playerId: game.reservation.playerId,
      reservedAt: game.reservation.reservedAt.toISOString(),
...
async function requirePlayerProfile(database: Database, playerId: string): Promise<void> {
  ...
  if (!player) {
    throw new GameError('Your player profile is still being created. Please retry in a moment.', 409);
  }
}
...
async function lockEligibleGame(database: Prisma.TransactionClient, playerId: string): Promise<string | null> {
  const candidate = await database.$queryRaw<Array<{ id: string }>>`
    ...
    where g.status = 'active'
      and (gp.player_id is null or (
        gp.status <> 'timed_out'
        and (gp.last_move_ply is null or g.current_ply - gp.last_move_ply >= g.move_distance)
      ))
      and not exists (
        select 1 from public.game_reservations gr
        where gr.game_id = g.id and gr.expires_at > now()
      )
...
export const gamesService = {
  async claimPlayableGame(playerId: string) {
    return prisma.$transaction(async (database) => {
      ...
      if (!candidateId) {
        ...
        const game = await database.game.create({
          data: {
            ...
            reservation: { create: { playerId, expiresAt } },
          },
          include: gameDetails,
        });
      ...
      await database.gameParticipant.upsert({
        where: { gameId_playerId: { gameId: candidateId, playerId } },
        create: { gameId: candidateId, playerId },
        update: {},
      });
  ...
  async submitMove(input: { gameId: string; playerId: string; from: string; to: string; promotion?: string | undefined; expectedVersion: number }) {
    return prisma.$transaction(async (database) => {
      await clearExpiredReservations(database);
      const locks = await database.$queryRaw<Array<{ id: string }>>`
        select id from public.games where id = ${input.gameId}::uuid for update
      `;
      ...
      if (!game.reservation || game.reservation.playerId !== input.playerId || game.reservation.expiresAt <= new Date()) {
        throw new GameError('Your five-minute move window has ended.', 409);
      }

      const participant = game.participants.find((item) => item.playerId === input.playerId);
      const isEligible = participant?.status !== 'timed_out'
        && (participant?.lastMovePly === null || participant?.lastMovePly === undefined
          || game.currentPly - participant.lastMovePly >= game.moveDistance);
      if (!isEligible) throw new GameError('You are not eligible to move on this board yet.', 403);

      const chess = new Chess(game.currentFen);
      let move;
      try {
        move = chess.move({ from: input.from, to: input.to, ...(input.promotion ? { promotion: input.promotion } : {}) });
      } catch {
        throw new GameError('That is not a legal chess move.', 422);
      }
      ...
      await database.gameParticipant.update({
        where: { gameId_playerId: { gameId: game.id, playerId: input.playerId } },
        data: { status: 'moved', lastMovePly: nextPly, timedOutAt: null },
      });
  ...
  async abortFirstMoveGame(gameId: string, playerId: string) {
    return prisma.$transaction(async (database) => {
      const locks = await database.$queryRaw<Array<{ id: string }>>`
        select id from public.games where id = ${gameId}::uuid for update
      `;
      if (locks.length === 0) return;
      const game = await getGameDetails(database, gameId);
      const canAbort = game.creatorId === playerId
        && game.currentPly === 0
        && game.reservation?.playerId === playerId
        && game.reservation.expiresAt > new Date();
